...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with open(os.path.join(str(settings.ROOT_DIR), "coworker/main/management/commands/resources/places.csv")) as urls_file:
            ...
            for _ in range(100):
                ...
                if place not in downloaded:
                    downloaded.append(place)
                    try:
    ...
    def parse_place(self, url):
        ...
        if Place.objects.filter(space_name=res["space_name"]).exists():
            return
