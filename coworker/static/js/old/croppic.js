...
(function (window, document) {
	...
	Croppic = function (id, options) {
		...
		that.options = {
			...
			loaderHtml:'<div class="loader" id="loadingProgressN"><div id="loadingProgressG"><div id="loadingProgressG_1" class="loadingProgressG"></div></div></div>',
			scaleToFill: false,
			processInline: false,
			loadPicture:'',
			onReset: null,
	...
	Croppic.prototype = {
		...
		createImgUploadControls: function(){
			...
			that.outputDiv.append('<div id="customHead" class="cropHead">&nbsp;</div>');
		...
		bindImgUploadControl: function(){
			...
			if( !$.isEmptyObject(that.croppedImg)){
				...
				that.cropControlRemoveCroppedImage.on('click',function(){ 
					...
					else {$('#'+that.options.customUploadButtonId).show();}
		...
		loadExistingImage: function(){
			...
			if( $.isEmptyObject(that.croppedImg)){
				...
				if(that.options.modal){	that.createModal(); }
				if( !$.isEmptyObject(that.croppedImg)){ that.croppedImg.remove(); }
				var loadImgUrl = that.options.loadPicture;
				$.ajax({
					url: that.options.reloadImgUrl,
					data: { headImgPrev: loadImgUrl, sw:that.objW, sh:that.objH, scwuserid:that.options.uploadData.cwuser_id},
					context: document.body,
					dataType: 'json',
					cache: false,
					type: 'POST'
				}).always(function (data) {
					if(data.loadImgUrl == undefined) {
						that.hideLoader();
						that.reset();
						alert('Image not Found');
					} else {
						that.imgUrl = data.loadImgUrl;
						var img =$('<img src="'+ data.loadImgUrl +'?timestamp='+new Date().getTime()+'">');
						that.obj.append(img);
						img.load(function() {
		...
		initialScaleImg:function(){
			...
			if(that.options.imgEyecandy){ that.imgEyecandy.css({'left': 0, 'top': 0, 'position':'relative'}); }
		...
		zoom :function(x){
			...
			/*if(newWidth > that.imgInitW || newHeight > that.imgInitH){*/
				...
				/*var ratioX = (that.imgW+x)/that.imgInitW;
                var ratioY = (that.imgH+x)/that.imgInitH;
                var ratioCal = Math.min(ratioX,ratioY);console.log(that.imgW+x/that.imgInitW);*/
                if(x>0) {
					newWidth  = that.imgW+(that.imgW * 0.1);
					newHeight = that.imgH+(that.imgH * 0.1);
					that.actualZoom += 1;
                } else {
                	newWidth  = that.imgW-(that.imgW * 0.1);
					newHeight = that.imgH-(that.imgH * 0.1);
		...
		crop:function(){
			...
			var cropData = {
					...
					zoomF:that.actualZoom,
					scview:that.options.uploadData.sview,
					scwuserid:that.options.uploadData.cwuser_id
				};
		...
		afterCrop: function (data) {
            ...
            if (response.status == 'success') {
                ...
                that.obj.append('<img class="croppedImg" src="' + response.url + '?timestamp='+new Date().getTime()+'">');
