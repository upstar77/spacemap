...
# from coworker.lab.comparison import EntryPublishedVectorBuilder
from coworker.lab.managers import HIDDEN
from coworker.lab.managers import PUBLISHED
...
class EntryAdmin(admin.ModelAdmin):
    ...
    list_filter = (CategoryListFilter, 'publication_date', 'status')
    ...
    def get_queryset(self, request):
        ...
        """
        if not request.user.has_perm('coworker.lab.can_view_all'):
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if not request.user.has_perm('coworker.lab.can_change_status'):
            readonly_fields.append('status')

        if not request.user.has_perm('coworker.lab.can_change_author'):
    ...
    def get_actions(self, request):
        ...
        if not actions:
            return actions
        if (not request.user.has_perm('coworker.lab.can_change_author') or
                not request.user.has_perm('coworker.lab.can_view_all')):
